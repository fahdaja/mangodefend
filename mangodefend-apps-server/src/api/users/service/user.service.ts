...
import { application_type, os_type, normalizeOsType, normalizeAppType } from '../enum/devices.enum';
...
export class UserService {
  ...
  async createUser(createUserDto: CreateUserDto): Promise<User> {
    ...
    if (existingUser) {
      ...
      // Cek apakah token OTP di Redis masih ada/aktif atau sudah hangus (>5 menit)
      const activeOtpToken = await this.redisService.getVerificationToken(createUserDto.email);
      const isExpiredByTime = (Date.now() - new Date(existingUser.createdAt).getTime()) > 300 * 1000;

      if (!activeOtpToken || isExpiredByTime) {
        // OTP sudah hangus: Hapus data pendaftaran unverified yang lama secara bersih
        await this.subscriptionRepository.delete({ user_id: existingUser.id });
        await this.deviceRepository.delete({ user_id: existingUser.id });
        await this.userRepository.remove(existingUser);
      } else {
        // OTP masih aktif: Minta user menyelesaikan verifikasi dengan OTP yang sudah dikirim
        throw new ConflictException(
          'Email ini sudah pernah terdaftar dan kode OTP verifikasi masih aktif. Silakan cek email Anda.',
        );
      }
    }
    ...
    try {
      ...
      if (freePlan) {
        ...
        end_date.setDate(end_date.getDate() + freePlan.durationDays);
      ...
      console.error('Failed to assign free plan on user registration:', err);
    }

    // Send Registration Email OTP Verification Code
    try {
      const otpCode = Math.floor(100000 + Math.random() * 900000).toString();
      await this.redisService.setVerificationToken(savedUser.email, otpCode, 300);
      await this.mailService.sendVerificationEmail(savedUser.email, otpCode);
    } catch (err) {
  ...
  async recordUserDevice(
    ...
    const normalizedData = {
      ...deviceData,
      os_type: normalizeOsType(deviceData.os_type),
      app_type: normalizeAppType(deviceData.app_type),
    };

    const existingDevice = await this.deviceRepository.findOne({
      where: { user_id: userId, hardware_id: normalizedData.hardware_id },
    });

    const now = new Date();

    if (existingDevice) {
      Object.assign(existingDevice, normalizedData);

      if (normalizedData.is_active === true) {
  ...
  async findOrCreateFirebaseUser(data: {
    ...
    try {
      ...
      console.error('Failed to assign free plan on Firebase user registration:', err);
    }
