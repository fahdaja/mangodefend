...
class MangoDefendLoadTest(HttpUser):
    ...
    def on_start(self):
        ...
        paths_to_try = [
            ...
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "mangodefend-ml-server", "Screenshot From 2026-05-31 15-08-43.png"),
        ]
    ...
    def task_scan_file(self):
        """
        Simulates scanning a file. High weight (70%) because this is the primary high-load endpoint.
        """
        files = {
        ...
        with self.client.post("/api/v1/scans/scan", files=files, data=data, catch_response=True) as response:
    ...
    @task(20)
    def task_get_logs(self):
        """
        Simulates checking the scan history page with pagination. Medium weight (20%).
        """
        page = random.randint(1, 5)
        limit = random.choice([10, 20, 50])
        label_filter = random.choice([None, "benign", "malware"])
        
        params = {"page": page, "limit": limit}
        if label_filter:
            params["label"] = label_filter

        with self.client.get("/api/v1/scans/logs", params=params, catch_response=True) as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"HTTP {response.status_code} on GET /logs: {response.text}")

    @task(8)
    def task_get_stats(self):
        """
        Simulates loading the dashboard statistics charts. Weight (8%).
        """
        with self.client.get("/api/v1/scans/stats", catch_response=True) as response:
            if response.status_code == 200:
                response.success()
            else:
                response.failure(f"HTTP {response.status_code} on GET /stats: {response.text}")

    @task(2)
    def task_health_check(self):
        """
        Simulates load balancers checking service health. Weight (2%).
        """
        with self.client.get("/health", catch_response=True) as response:
            if response.status_code == 200:
                try:
                    ...
                    response.failure("Failed to parse health response as JSON")
            else:
                response.failure(f"HTTP {response.status_code} on GET /health: {response.text}")