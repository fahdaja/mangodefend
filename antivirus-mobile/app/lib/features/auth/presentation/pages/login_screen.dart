...
import 'package:antivirus_mobile/features/auth/presentation/pages/register_screen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _handleEmailLogin() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (response.success) {
          final user = response.user ?? UserModel(
            id: 1,
          ...
          UserSession.login(
            ...
            refreshTokenVal: response.refreshToken,
          );
  ...
  void _handleGoogleOAuth() async {
    ...
    try {
      final googleResult = await NativeGoogleAuthService.signIn(context);
      ...
      if (mounted) {
        ...
        if (response.success) {
          final user = response.user ?? UserModel(
            id: 1,
            username: googleResult.fullName,
            email: googleResult.email,
          );
          UserSession.login(
            user,
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Berhasil masuk via Google (${user.email})'),
              backgroundColor: const Color(0xFF10B981),
            ),
          );
          widget.onLoginSuccess?.call();
                            ...
                            children: [
                              ...
                              GestureDetector(
                                onTap: () {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text('Fitur reset password dikirim ke email Anda.'),
                                    ),
                                  );
                                },
                                child: const Text(
